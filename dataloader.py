...
class CustomDataset(Dataset):
    def __init__(self, data_dir, split="train", device="cuda", index_file=None):
        ...
        if len(camera_dir) > 1:
            ...
            self.camera_info = json.load(
                open(os.path.join(data_dir, "camera_infos.json"))
            )
            ...
            for camera_name in camera_dir:
                num_images[camera_name] = len(
                    os.listdir(os.path.join(images_dir, camera_name))
                )
                image_paths = [
                    os.path.join(images_dir, camera_name, i)
                    for i in os.listdir(os.path.join(images_dir, camera_name))
                    if i.endswith(".png") or i.endswith(".jpg") or i.endswith(".jpeg")
                ]
                if index_file:
                    image_paths.sort()
                    moving_indices = np.load(os.path.join(data_dir, index_file))
        ...
        if split == "val":
            self.image_paths = np.random.choice(self.image_paths, size=5, replace=False)
    ...
    def __getitem__(self, index):
        if self.camera_param_type == "fov":
            ...
            mask = torch.ones(gt_image.shape[-3:-1]).to(self.device)
            ...
            intrinsic = (
                torch.tensor(self.camera_info[camera_name]["intrinsic"])
                .unsqueeze(0)
                .to(self.device)
            )
            transform = (
                torch.tensor(self.camera_info[camera_name]["transform"])
                .unsqueeze(0)
                .to(self.device)
                .float()
            )
            image_size = (
                torch.tensor(gt_image.shape[-3:-1]).unsqueeze(0).to(self.device)
            )
            image_idx = int(image_path.split("/")[-1].split(".")[0])
            final_transform = (
                transform @ torch.tensor(self.poses[image_idx]).to(self.device).float()
            )
...
def get_dataloader(data_dir, batch_size=1, device="cuda", index_file=None):
    ...
    dataset_train = CustomDataset(
        data_dir, split="train", device=device, index_file=index_file
    )
    dataset_val = CustomDataset(
        data_dir, split="val", device=device, index_file=index_file
    )
    print(
        "Train dataset size: ",
        len(dataset_train),
    ...
    dataloader_val = DataLoader(
        dataset_val, batch_size=batch_size, shuffle=False, collate_fn=collate_fn
    )
    return dataloader_train, dataloader_val, image_shape


if __name__ == "__main__":
    train_loader, val_loader, image_shape = get_dataloader(
        "/home/zhang/hdd/data_texture_optimization/campus_garching"
    )
