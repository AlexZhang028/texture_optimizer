...
- `camera_infos.json` saves the intrinsic matrix and transformation matrix relative to the LiDAR.
- `indices.npy` saves the indices of the images used for training.
...
- `masks/` contains the masks of moving object used for training. These images should be named sequentially.
...
- `--mesh_name`: Name of the mesh file (with extension).
- `--device`: Device to run the training on ("cuda" for GPU or "cpu").
- `--epochs`: Number of epochs to train (default: 10).
- `--lr`: Learning rate (default: 0.01).
- `--texture_size`: Size of the texture image (default: 512).
- `--hierarchical_texture`: Flag to use hierarchical texture optimization (default: False).
- `--hierarchical_layers`: Number of levels in the hierarchical texture optimization (default: 4).
- `--index_file`: Name of file containing the indices of the images used for training (default: indecies.npy).
...
4. **Saving**: At the end of training, the optimized texture is saved as `{mesh_name}.png` in the specified save directory.
